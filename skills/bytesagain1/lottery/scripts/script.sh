#!/usr/bin/env bash
# lottery — tags: [lottery, tool, utility]
# Powered by BytesAgain | bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${LOTTERY_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/lottery}"
...
cmd_init() {
    ensure_dirs
    echo "[lottery] Initialized at $DATA_DIR"
    echo "  Entries file: $ENTRIES"
    echo "  Config file:  $CONFIG"
    echo ""
    echo "Ready. Try: lottery add \"your first item\""
}

cmd_add() {
    ensure_dirs
    local text="${*:?Usage: lottery add <text>}"
    local ts=$(now_ts)
    local id=$(($(entry_count) + 1))
    printf '{"id":%d,"text":"%s","created":"%s"}\n' "$id" "$text" "$ts" >> "$ENTRIES"
    echo "[lottery] #$id added: $text"
}
...
cmd_list() {
    ...
    if [ "$total" -eq 0 ]; then
        echo "[lottery] No entries yet. Use: lottery add <text>"
        return
    fi
    echo "[lottery] $total entries:"
    echo ""
    local n=0
...
cmd_remove() {
    ensure_dirs
    local id="${1:?Usage: lottery remove <id>}"
    local total=$(entry_count)
    if [ "$id" -lt 1 ] || [ "$id" -gt "$total" ]; then
        echo "Error: id must be 1-$total"; return 1
    fi
    ...
    echo "[lottery] Removed #$id: $text"
}

cmd_search() {
    ensure_dirs
    local term="${1:?Usage: lottery search <term>}"
    local found=0
    local n=0
    while IFS= read -r line; do
        n=$((n + 1))
    ...
    echo "[lottery] Found $found matches for '$term'"
}
...
case "${1:-help}" in
    ...
    version|-v)     echo "lottery v$VERSION" ;;
