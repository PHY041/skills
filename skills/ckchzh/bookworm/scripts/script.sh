#!/usr/bin/env bash
# bookworm — description: "Reading list and book tracker with progress, ratings, and recommen
# Powered by BytesAgain | bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${BOOKWORM_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/bookworm}"
...
cmd_init() {
    ensure_dirs
    echo "[bookworm] Initialized at $DATA_DIR"
    echo "  Entries file: $ENTRIES"
    echo "  Config file:  $CONFIG"
    echo ""
    echo "Ready. Try: bookworm add \"your first item\""
}

cmd_add() {
    ensure_dirs
    local text="${*:?Usage: bookworm add <text>}"
    local ts=$(now_ts)
    local id=$(($(entry_count) + 1))
    printf '{"id":%d,"text":"%s","created":"%s"}\n' "$id" "$text" "$ts" >> "$ENTRIES"
    echo "[bookworm] #$id added: $text"
}
...
cmd_list() {
    ...
    if [ "$total" -eq 0 ]; then
        echo "[bookworm] No entries yet. Use: bookworm add <text>"
        return
    fi
    echo "[bookworm] $total entries:"
    echo ""
    local n=0
...
cmd_remove() {
    ensure_dirs
    local id="${1:?Usage: bookworm remove <id>}"
    local total=$(entry_count)
    if [ "$id" -lt 1 ] || [ "$id" -gt "$total" ]; then
        echo "Error: id must be 1-$total"; return 1
    fi
    ...
    echo "[bookworm] Removed #$id: $text"
}

cmd_search() {
    ensure_dirs
    local term="${1:?Usage: bookworm search <term>}"
    local found=0
    local n=0
    while IFS= read -r line; do
        n=$((n + 1))
    ...
    echo "[bookworm] Found $found matches for '$term'"
}
...
case "${1:-help}" in
    ...
    version|-v)     echo "bookworm v$VERSION" ;;
