...
特定版本，示例：https://docs-pd.mingdao.com/upgrade/7.0.0/
```
...
- **架构兼容性红线**：即便支持跨版本升级，也必须确保目标版本有对应的 CPU 架构镜像。若总表未标记支持，不得继续生成文档。
...
从中识别：
- **架构严格校验（最高优先级）**：
    - 必须以 `https://docs-pd.mingdao.com/version` 总表中的 ✅ 标记为准。
    - 若目标版本在对应架构列（AMD64/ARM64）为**空白、❌ 或未标记**，必须视为**不支持**。
    - **思考链断言要求**：在生成文档前，必须在思考记录中明确写出：“正在核验 v{目标版本} 在 {架构} 下的标记，实际值为 {取值}，判定结果为 {支持/不支持}”。
    - **严禁**因为在详情页或部署指南中看到通用的 `arm64` 拉取指令、Tab 选项卡而推断该版本支持该架构。
    - **立即中止规则**：若不支持（即标记不等于 ✅），必须立即告知用户：“抱歉，目标版本 {版本号} 官方尚未发布 {架构} 镜像（以发布历史总表为准），升级文档生成任务已中止。”
...
Pre 合并：镜像命名替换执行一次（v7.1.0）、MongoDB 库一次性全部创建、MongoDB 预置数据更新至含有该操作的最新版本（如路径中只有 v7.0.0 有此操作，则更新至 v7.0.0）、yaml 新增服务合并写入
...
**MongoDB 预置数据更新**（仅执行流程中包含此操作的最新版本，4 种情况）
...
1. **提前准备（在可联网机器下载）**：
   ...
   `wget https://pdpublic.mingdao.com/private-deployment/data/preset_mongodb_{该操作涉及的最新版本号}.tar.gz`
2. **执行升级（在服务器执行）**：
